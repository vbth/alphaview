...
export function renderDashboardList(data, container, eurUsdRate) {
    ...
    // Header
    let html = `
        <div class="grid grid-cols-12 gap-2 px-2 md:px-6 py-3 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-500 uppercase tracking-wider">
            <div class="col-span-4 md:col-span-3 text-left"><i class="fa-solid fa-layer-group"></i> <span class="hidden md:inline">Wertpapier</span><span class="md:hidden">Asset</span></div>
            <div class="hidden md:block md:col-span-2 text-right"><i class="fa-solid fa-tag"></i> Preis</div>
            <div class="col-span-3 md:col-span-2 text-right"><i class="fa-solid fa-chart-line"></i> Perf.</div>
            <div class="col-span-3 md:col-span-2 text-right"><i class="fa-solid fa-coins"></i> Wert</div>
            <div class="col-span-2 md:col-span-1 text-right"><i class="fa-solid fa-hashtag"></i> Stück</div>
            <div class="hidden md:block md:col-span-1 text-right"><i class="fa-solid fa-chart-pie"></i> Ant.</div>
            <div class="hidden md:block md:col-span-1 text-right"><i class="fa-solid fa-bolt"></i></div>
        </div>
    `;
    ...
    data.forEach(item => {
        ...
        <div class="grid grid-cols-12 gap-2 px-2 md:px-6 py-4 border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors items-center group stock-card" data-symbol="${item.symbol}">
            ...
            <div class="hidden md:block md:col-span-2 text-right font-mono text-sm text-slate-700 dark:text-slate-300">
                ${formatMoney(item.price, item.currency)}
            </div>

            <div class="col-span-3 md:col-span-2 text-right font-mono text-xs md:text-sm font-medium ${colorClass}">
                ${formatPercent(item.changePercent)}
            </div>

            <div class="col-span-3 md:col-span-2 text-right font-mono font-bold text-slate-900 dark:text-white text-xs md:text-sm">
            ...
            <div class="hidden md:flex md:col-span-1 text-right justify-end gap-2">
